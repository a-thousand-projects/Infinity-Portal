...
Learn more at [https://github.ATPrograms/ATPRotary]
...
ATPRotary::ATPRotary(byte pinA, byte pinB, byte pinS)
{
  _pinA = pinA;
  _pinB = pinB;
  _pinS = pinS;
...
/**
	...
	@since vATP*/
void ATPRotary::setTrigger(byte i)
{
  _trigger = i;
  _setInputPins();
}
...
/**
	...
	@since vATP*/
void ATPRotary::setDebounceDelay(int i)
{
  _debounceRDelay = i;
}
...
/**
	...
	@since vATP*/
void ATPRotary::setErrorDelay(int i)
{
  _errorDelay = i;
}
...
/**
	...
	@since v0.1;
	@return byte, value of turned kATP*/
byte ATPRotary::push(){
	_updateTime();
	byte val = 0x00;
	...
	if (val ==1 && _buttonPushCallback != nullptr)
	{
		_buttonPushCallback();
	}
...
void ATPRotary::setPushButtonCallBack(ButtonPushCallback callback)
{
	_buttonPushCallback = callback;
}
...
/**
	...
	@return int, time in MS that the button has been held ATP**/
int ATPRotary::pushTime(){
	unsigned int t = 0;
	//byte s = push();
	if ( !_statusS &&  !_statusS_prev ) {
...
/**
	...
	@return ATP*/
byte ATPRotary::pushType(int i = 1000){
	_updateTime();
	_statusS = ( digitalRead(_pinS) == _trigger ) ? true : false;
	byte val = 0x00;
